...
def gradient_descent_runner(points, starting_b, starting_m, learning_rate, num_iterations):
    ...
    m = starting_m
    rss_array = []
    rss_file = open("rss_1000.csv", "w")

    for i in range(num_iterations):
        b, m = step_gradient(b, m, array(points), learning_rate)
        #print "RSS = {0} for iteration number {1}".format(calculate_rss(b, m, points), i)
        squared_error = calculate_rss(b, m, points)
        rss_array.append(squared_error)
        rss_file.write("{0},{1}\n".format(i,squared_error))

    print "RSS values for {0} iterations are {1}.".format(num_iterations, rss_array)
    return [b, m]

def run():
    points = genfromtxt("school-data.csv", delimiter=",")
